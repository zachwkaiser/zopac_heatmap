...
class Shipper:
    def __init__(self, server_url, api_key, endpoint_id, batch_size, flush_ms, timeout_s, use_gzip=True):
        self.url = server_url
    ...
    def _post(self, records):
        if not self.url or not records:
            return True
        body = json.dumps({"endpoint_id": self.endpoint_id, "records": records}).encode()   # Include endpoint_id in the request body
        data = gzip.compress(body) if self.use_gzip else body
        backoff = 0.5
        for _ in range(5):
            try:
                r = self.session.post(self.url, data=data, headers=self.headers, timeout=self.timeout)
