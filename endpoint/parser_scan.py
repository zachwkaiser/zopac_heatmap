...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Parse tcpdump output to JSONL (mac, rssi, ts).")
    parser.add_argument("--from", dest="source", default=None)
    parser.add_argument("--out", dest="out_path", default="-")
    args = parser.parse_args()

    out = sys.stdout if args.out_path == "-" else open(args.out_path, "w", encoding="utf-8")
    try:
