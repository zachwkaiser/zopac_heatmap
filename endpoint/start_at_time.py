...
def is_ntp_synced():
    ...
    try:
        out = subprocess.check_output(["timedatectl", "show", "-p", "NTPSynchronized"], text=True).strip()
...
def parse_time(tstr):
    ...
    try:
        ...
        return datetime.datetime.fromisoformat(tstr).replace(tzinfo=datetime.timezone.utc)
    except Exception as e:
        raise ValueError(f"Invalid time format: {tstr} -- use YYYY-MM-DDTHH:MM:SSZ") from e
...
def wait_until(target_dt, check_ntp=True, ntp_timeout=60):
    ...
    if check_ntp:
        ...
        while not is_ntp_synced():
            if time.time() - start > ntp_timeout:
                print("Warning: NTP not synced after timeout; proceeding anyway.", flush=True)
                break
    ...
    if delta <= 0:
        ...
        return
    print(f"Sleeping until target UTC time {target_dt.isoformat()} (sleep {int(delta)}s)...", flush=True)
...
def main():
    ...
    parser.add_argument("--time", required=True, help="Target UTC time (ISO) e.g. 2025-10-25T12:00:00Z")
    parser.add_argument("--capture-script", default="/home/pi/WiFi_Project/capture_wifi.sh",  # safer absolute path
                        help="Path to capture script")
    parser.add_argument("--iface", default="wlan1", help="Wireless interface")
    parser.add_argument("--logdir", default="/home/pi/WiFi_Project/logs", help="Directory for logs")
    parser.add_argument("--channel", default=None, help="Channel to set before capture (optional)")
    parser.add_argument("--duration-sec", type=int, default=0,  # <- add this line
                        help="Auto-stop after N seconds (optional)")
    parser.add_argument("--no-ntp-check", action="store_true", help="Do not wait for NTP sync")
